...
#                print '+',str(ele),'-'
    ...
    def getMainArticle(self):
        ...
        tags_all = soup.find_all('div', {'class': 'article_title'})  # 修改此行
        self.getArticleName(tags_all)
        tags_all = soup.find_all('div', {'id': 'article_content', 'class': 'article_content'})  # 修改此行
    ...
    def chineseListOut(self, tags):
        ...
        for tag in tags:
            for ele in tag:
                title.append(ele.strip())
        if len(title) > 1:  # 添加检查以避免索引错误
            self.link = "http://blog.csdn.net" + title[1]
        if len(title) > 2:  # 添加检查以避免索引错误
            tle = title[2].split()
            self.articleName = '-'.join(tle)
    ...
    def saveArticleAsHTML(self):
        ...
        try:
            ...
            print('open error')
        print('path = ',filePath)
        filePointer.write(str(self.HTML))
        filePointer.close()

browser = BrowserBase()
url = input('Input the links of CSDN article you needed!\n')
